function Invoke-PublishModules {
    ...
    if ([version]$ReportingGithub -gt [version]$ReportingPSGallery ) {
        ...
        Publish-Module -Path "$workingdirectory/modules/MSP365.Reporting" -NuGetApiKey $env:NUGET_KEY
    ...
    if ([version]$SAMGithub -gt [version]$SAMPSGallery ) {
        ...
        Publish-Module -Path "$workingdirectory/modules/MSP365.SAM" -NuGetApiKey $env:NUGET_KEY
    ...
    if ([version]$MSP365Github -gt [version]$MSP365PSGallery ) {
        ...
        Publish-Module -Path "$workingdirectory/modules/MSP365" -NuGetApiKey $env:NUGET_KEY
